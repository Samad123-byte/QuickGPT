...
const {chats, setSelectedChat, theme, setTheme, user, navigate, createNewChat, axios, setChats, fetchUserChats, 
  setToken, token
} = useAppContext()
...
<input onChange={(e) =>setSearch(e.target.value)} value={search} type="text" placeholder='Search conversations' className='text-xs placeholder:text-grey-400 outline-none'/>
        </div>
        {/*chats*/}
        ...
        chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
          <div onClick={()=> {navigate('/'); setSelectedChat(chat); setIsMenuOpen(false)}}
           key={chat._id} className='p-2 px4 dark:bg-[#57317C]/10 border border-grey-300 dark:border-[#80609F]/15
        ...
        {/*community images */}
        <div onClick={() => {navigate('/community'); setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-4 border border-grey-300 
        ...
        <div onClick={() => {navigate('/credits'); setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-4 border border-grey-300 
        dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        {/*dark mode*/}
         <div className='flex items-center justify-between gap-2 p-3 mt-4 border border-grey-300 
   ...
   {/*User account*/}
  <div className='flex items-center gap-3 p-3 mt-4 border border-grey-300 
